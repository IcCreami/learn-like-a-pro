...
| 章节 | 行数 | 内容 |
|------|------|------|
| YAML Frontmatter | 1-16 | name + description（pushy 风格，英文，给 AI 看） |
| 角色定义 | 20-45 | Sensei 角色、四条黄金规则、标志性语气 |
| 触发条件 | 46-65 | 激活场景（想学/小白/困惑/续接/专业 skill 协作信号） |
| 状态管理 | 66-160 | 首次学习 vs 续接学习的逻辑、state.json 字段说明、产物生成规则、跨对话复用 |
| 核心工作流 | 161-377 | 六步循环详解（诊断→地图→知识构建→实践→回忆→产物） |
| 教学原则 | 391-402 | 7 条铁律 |
| 质量检查清单 | 403-429 | 首次检查 / 每单元必检 / 产物检查 / 通用检查 |
| 特殊场景 | 430-560 | 6 个场景（懂了跳过/卡住3次/时间到了/超纲/迷茫/专业 skill 协作） |
| 示例对话 | 561-675 | 3 个完整示例（学 AI Agent / 迷茫 / 写 Skill） |
| 会话结束模板 | 676-713 | 标准化结束语 |
| 设计灵感来源 | 714-723 | 7 个参考 Skill 的致谢 |
...
| 章节 | 行数 | 内容 |
|------|------|------|
| 角色定义 | 1-31 | Sensei 角色、四条黄金规则、标志性语气（同 SKILL.md） |
| 续接流程 | 32-41 | 读取 state.json → 确认续接 → 从第三步开始 |
| 知识构建 | 42-90 | 费曼导入、苏格拉底检验、支架练习 |
| 动手实践 | 91-106 | 实践要求、报错处理三步走 |
| 主动回忆 | 107-120 | 3 题测试、评分标准 |
| 产物生成 | 121-153 | 产物更新策略、state.json 更新字段 |
| 会话结束模板 | 154-180 | 标准化结束语（精简版） |
