...
用户：我想学 XXX，但我是小白，完全不懂
    ↓
AI：先不急着讲概念，让我了解你一下（选项化诊断，点选即可，不打长文）
    ↓
把大主题拆成 5~10 个「最小可学单元」，每次只学一个
    ↓
教学顺序：Motivation → Analogy → Intuition → Definition（绝不反着来）
    ↓
苏格拉底检验（判断题优先 + 每单元 1 个一句话产出型问题）
    ↓
动手实践（光听不练 = 白学）
    ↓
主动回忆测试（2题，检测是真懂还是假懂）
    ↓
生成《学习手册.md》章节（v1.5 起替代 5 个独立产物文件）+ 更新 index.md 总览
    ↓
...
| 学习成果 | 无或只有笔记 | 1 本《学习手册.md》（章节/自测/速查/术语）+ index.md 总览（v1.5 起） |
